...
export default async function MySmes() {
  ...
  return (
    <main style={{ maxWidth: 820, margin: '3rem auto', padding: '0 1.5rem', lineHeight: 1.6 }}>
      ...
      <p style={{ color: 'var(--app-muted)', marginTop: 4, display: 'flex', gap: '1rem', alignItems: 'baseline' }}>
      ...
      {flash && (
        <div style={{ background: flash.startsWith('Error') ? 'var(--app-danger-bg)' : 'var(--app-ok-bg)',
                      border: '1px solid var(--app-line)', borderRadius: 8, padding: '0.7rem 1rem', margin: '1rem 0',
      ...
      <section style={card}>
        ...
        <p style={{ fontSize: '0.9rem', color: 'var(--app-muted)', marginTop: 0 }}>
      ...
      <p style={{ color: 'var(--app-muted)', fontSize: '0.85rem', marginTop: 0 }}>
...
function Field({ name, label, required, placeholder }) {
  return (
    <label style={{ fontSize: '0.85rem', color: 'var(--app-ink)' }}>
      {label}
...
const card = { border: '1px solid var(--app-line)', borderRadius: 10, padding: '1rem 1.25rem', margin: '1.25rem 0' };
const h2 = { marginTop: 0, fontSize: '1.1rem' };
const row2 = { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.7rem' };
const input = { display: 'block', width: '100%', marginTop: 3, padding: '0.45rem', border: '1px solid var(--app-line)', borderRadius: 6, boxSizing: 'border-box' };
...
const linkBtn = { background: 'none', border: 'none', color: 'var(--app-muted)', cursor: 'pointer', textDecoration: 'underline', padding: 0, fontSize: '0.82rem' };
